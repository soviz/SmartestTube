...
* Do not spawn subagents (Agent tool / Explore / general-purpose / Task-style delegation) unless the user explicitly asks for one. Offer to use subagents if it's really heavy task. Subagent-heavy sessions disproportionately burn through usage limits — handle searches and multi-step work directly with your own tools (grep/find/Read) instead of delegating.
...
* Keep in-chat output minimal while working: don't paste changed code/diffs into the chat, and don't narrate step-by-step reasoning as you go. Give short status updates only at key moments (found something, changed direction, blocked). Save the explanation of what was done for a single summary at the end of the response — one summary per turn, not one per step.
* When investigating a bug or method, use grep/search to locate the relevant method first; for a quick lookup or small file, reading just the surrounding lines via offset/limit is fine — but read the whole file (or class) instead whenever you're about to change a method's signature/behavior in a way visible outside it, the file is small enough that partial reading saves little anyway, or you're unsure whether sibling methods/fields/overrides affect the change. When in doubt, read more, not less — a missed call site or an overridden method costs far more (in tokens and correctness) than an extra file read.
* Prefer targeted reads over reading unrelated files "just in case" — but don't skip a file that plausibly interacts with the change (parent/child classes, callers, shared state) just to save tokens. If unsure whether a file matters, read it rather than guessing.
* When searching the codebase, start with the narrowest plausible search (exact symbol/string), but broaden immediately if the narrow search doesn't clearly account for all usages — especially before renaming/removing a method or changing a public API, where missing a call site is worse than an extra grep.
* When a task touches many files, list which files you intend to change before editing them, then edit silently — do not paste intermediate diffs or "now editing X" narration per file.
